  ...
  Widget _buildReportContent(MonthlyReport report) {
    return Column(
      ...
      children: [
        ...
        Row(
          children: [
            Expanded(child: ReportStatCard(title: 'Kutilgan', value: '${report.expectedRevenue.toStringAsFixed(0)}', subtitle: 'so\'m', icon: Icons.trending_up_rounded, color: AppColors.primary)),
            const SizedBox(width: 12),
            Expanded(child: ReportStatCard(title: 'Haqiqiy', value: '${report.actualRevenue.toStringAsFixed(0)}', subtitle: 'so\'m', icon: Icons.account_balance_wallet_rounded, color: AppColors.success)),
          ],
        ),
        const SizedBox(height: 12),
        Row(
        ...
        // Group Statistics
        SectionHeader(title: 'Guruhlar statistikasi', count: report.groupStats.length),
        const SizedBox(height: 12),
        if (report.groupStats.isEmpty)
          const NoDataCard(message: 'Bu oy uchun guruh ma\'lumotlari yo\'q')
        else
      ...
      child: Theme(
        ...
        child: ExpansionTile(
          ...
          children: [
            const Divider(),
            const SizedBox(height: 8),
            InfoRow(label: 'Faol o\'quvchilar', value: '${group.activeStudents}'),
            InfoRow(label: 'Kutilgan daromad', value: '${group.expectedRevenue.toStringAsFixed(0)} so\'m'),
            InfoRow(label: 'Haqiqiy daromad', value: '${group.actualRevenue.toStringAsFixed(0)} so\'m', valueColor: AppColors.primary),
            InfoRow(label: 'Yig\'ish foizi', value: '${group.collectionRate.toStringAsFixed(1)}%', valueColor: AppColors.success),
            const SizedBox(height: 12),
            Row(
              children: [
                Expanded(child: MiniStatCard(label: 'To\'lagan', value: '${group.paidStudents}', color: AppColors.success)),
                const SizedBox(width: 8),
                Expanded(child: MiniStatCard(label: 'To\'lamagan', value: '${group.unpaidStudents}', color: AppColors.warning)),
              ],
            ),
          ],
        ),
