...
import 'inquiry_local_datasource.dart';
import 'inquiry_remote_datasource.dart';

class InquirySyncHandler implements SyncOperationHandler {
  final InquiryRemoteDataSource _remote;
  final InquiryLocalDataSource _local;

  InquirySyncHandler(this._remote, this._local);

  @override
  String get entityType => 'inquiry';

  @override
  Future<int?> execute(SyncOperation op, IdMappingService idMapping) async {
    switch (op.operationType) {
      case 'create':
        final request = InquiryRequest.fromJson(op.payload!);
